...
##
## Network Parameterization for CombPrev Project
##
...
d <- l %>%
  ...
  filter(ptype == 1) %>%
  group_by(AMIS_ID) %>%
  summarise(deg.main = sum(ONGOING)) %>%
...
d <- l %>%
  ...
  filter(ptype == 2) %>%
  group_by(AMIS_ID) %>%
  summarise(deg.casl = sum(ONGOING)) %>%
  right_join(d, by = "AMIS_ID")
...
count.oo <- ifelse(is.na(d$count.oo), 0, d$count.oo)
head(count.oo, 25)
summary(count.oo)

# define OO count = total cuml AI partners - MC partners reported on in Partner Module
head(d$ai.part, 25)
